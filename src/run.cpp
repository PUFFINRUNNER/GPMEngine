...
void Run::update_display_stats(Display *dsp) {
    dsp->update_stats();
}
...
void Run::start() {
    ...
    timer = al_create_timer(1.0 / (double)Run::FPS);
    if (!timer) {
    ...
    auto al_display = al_create_display(Run::WIDTH, Run::HEIGHT);
    ...
    std::cout << "[INFO]: Allegro successfully started" << std::endl;
    ...
    forever {
        ...
        Run::update_display_stats(display);
...
void Run::stop() {
    ...
    if (timer) {
        ...
        if (verbose)
            std::cout << "[ OK ]: timer destroyed" << std::endl;
    }

    if (display) {
        al_destroy_display(display->get_display());
        if (verbose)
            std::cout << "[ OK ]: display destroyed" << std::endl;
    }

    if (event_queue) {
        al_destroy_event_queue(event_queue);
        if (verbose)
            std::cout << "[ OK ]: event queue destroyed" << std::endl;
    }

    stopped = true;

    std::cout << "[INFO]: Allegro was successfully stopped" << std::endl;
}