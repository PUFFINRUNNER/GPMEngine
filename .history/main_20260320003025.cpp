...
namespace {
    ...
    public:
        ...
        void onUpdate(double dt) override {
            ...
            m_baseSpin += static_cast<float>(dt) * 0.35f;
        ...
        void onRender(const G::Viewport& viewport, double /*alpha*/) override {
            ...
            if (m_baseBody) {
                drawCubeFromBody(*m_baseBody, viewport, white, 2.0f, true);
            }
            if (m_dropBody) {
                drawCubeFromBody(*m_dropBody, viewport, white, 2.0f, false);
    ...
    private:
        ...
        void respawnDropCube() {
            ...
            m_dropBody->position = M::Point3D(0.0f, 4.5f, 0.0f);
            m_dropBody->rotation = M::Quaternion::identity();
        ...
        void drawCubeFromBody(const P::RigidBody& body,
                              const G::Viewport& viewport,
                              ALLEGRO_COLOR color,
                              float thickness,
                              bool animatedBase) const {
            M::Transform3D t = body.transform();
            if (animatedBase) {
                const M::Quaternion spin = M::Quaternion::fromAxisAngle(M::Vector3D(0.0f, 1.0f, 0.0f), m_baseSpin);
                t.rotation = spin * t.rotation;
            }

            const G::Mesh worldCube = m_cubeMesh.transformed(t);
        ...
        void drawHelp(const G::Viewport& viewport) {
            ...
            al_draw_text(m_font, white, 12.0f, static_cast<float>(viewport.height - 24), 0,
                         "Move camera: W A S D E Q | Physics: drop cube resets every 10s");
        }
    };
}
