...
class Vector2D {
  public:
    ...
    Vector2D operator/(floatx) const;   //divide magnitude by x
    Vector2D &operator*=(floatx);   //^
    Vector2D &operator/=(floatx);   //^
    Vector2D atMagnitude(floatmag) const;   //return a vector that has the same direction as this but with magnitude mag
    Vector2D setMagnitude(floatmag);    //set magnitude to mag
