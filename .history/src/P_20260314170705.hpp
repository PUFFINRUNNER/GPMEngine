...
class Vector2D {
  public:
    ...
    Vector2D &operator-=(const Vector2D &other);    //^
    Vector2D operator*(const float &other) const;   //multiply magintude by
    Vector2D operator/(const float &other) const;   //divide magnitude by
    Vector2D &operator*=(const float &other);   //^
    Vector2D &operator/=(const float &other);   //^
    Vector2D atMagnitude(const float &other) const;    /
