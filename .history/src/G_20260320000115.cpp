...
void recomputeSmoothNormals(Mesh& mesh) noexcept {
    ...
    [[nodiscard]] bool clipLineToNearPlaneView(
        M::Vector3D& a,
        M::Vector3D& b,
        float nearPlane) noexcept {
        // In this camera convention, forward is along -Z in view space.
        // Visible points satisfy z <= -nearPlane.
        ...
        if (std::fabs(ab.z) < M::EPS) {
            return false;
        }

        const float t = (nearZ - a.z) / ab.z;
        const M::Vector3D p = lerpVec3(a, b, t);
    ...
    [[nodiscard]] bool projectViewPointToScreen(
        const M::Vector3D& viewPoint,
        const M::Matrix4x4& proj,
        const G::Viewport& viewport,
        G::ScreenPoint3D& out) noexcept {
        const M::Vector4D clip = proj * M::Vector4D(viewPoint, 1.0f);
        ...
        // For points, require the final point to be inside clip space.
        out.valid = (ndcX >= -1.0f && ndcX <= 1.0f &&
                     ndcY >= -1.0f && ndcY <= 1.0f &&
                     ndcZ >= -1.0f && ndcZ <= 1.0f);
        return out.valid;
    }
