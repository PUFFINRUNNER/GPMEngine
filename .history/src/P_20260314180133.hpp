...
namespace P {
    ...
    public:
        ...
        float w;
        float eps;
        constexpr Vector2D(): v(0), w(0), eps(DEFAULT_EPS) {}
        constexpr Vector2D(float _eps): v(0), w(0), eps(_eps) {}
        constexpr Vector2D(float _v, float _w): v(_v), w(_w), eps(DEFAULT_EPS) {}
        constexpr Vector2D(float _v, float _w, float _eps): v(_v), w(_w), eps(_eps) {}
        ...
        constexprVector2D operator-(const Vector2D &other) const;    //subtract other vector
        Vector2D operator-() const; //instead of writing "negated"
        Vector2D &operator+=(const Vector2D &other);    //^
        Vector2D &operator-=(const Vector2D &other);    //^
        Vector2D operator*(float x) const;   //multiply magintude by x
        Vector2D operator/(float x) const;   //divide magnitude by x
