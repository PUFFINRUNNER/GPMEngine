...
namespace P {
    ...
    public:
        ...
        constexpr Vector2D operator/(float x) const noexcept {  //divide magnitude by x
            if (std::fabsl(x) < EPS) {
        ...
        float angleBetween(const Vector2D &other) const; //get the smallest not negative angle between 2 vectors
