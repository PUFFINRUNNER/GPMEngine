...
Transform2D Transform2D::combinedWith(const Transform2D& other) const noexcept {
    const Matrix3x3 m = matrix() * other.matrix();

    Transform2D out;
    out.position = Point2D(m.m[0][2], m.m[1][2]);
    out.rotation = std::atan2(m.m[1][0], m.m[0][0]);
    out.scale = Vector2D(
        std::sqrt(m.m[0][0] * m.m[0][0] + m.m[1][0] * m.m[1][0]),
        std::sqrt(m.m[0][1] * m.m[0][1] + m.m[1][1] * m.m[1][1])
    );
    return out;
}
