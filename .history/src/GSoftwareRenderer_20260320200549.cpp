...
namespace G {
    ...
    void SoftwareRenderer3D::rasterizeLine(const ScreenPoint3D& a,
        ...
        for (int i = 0; i <= steps; ++i) {
            ...
            const float xf = a.x + dx * t;
            const float yf = a.y + dy * t;const float zf = a.z + (b.z - a.z) * t - 1.0e-4f;

            const int x = static_cast<int>(std::lround(xf));
            const int y = static_cast<int>(std::lround(yf));

            putPixel(x, y, zf, color);
        }
    }
