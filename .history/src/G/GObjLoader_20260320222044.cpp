...
namespace {
    ...
    loadMtl(const std::string& mtlPath) {
        ...
        if (!file) {
            ...
            throw std::runtime_error("failed to load mtl");
            return materials;
        }
    ...
    std::cout << "albedo: "
              ...
              << material.albedo.g << " "
              << material.albedo.b << "\n";
    std::cout << "texture path: " << parsed->diffuseTexturePath << "\n";
}
...
namespace G {
    ...
    Model ObjLoader::load(const std::string& objPath) {
        ...
        if (!model.parts.empty() && model.parts[0].mesh && !model.parts[0].mesh->vertices.empty()) {
            const auto& v = model.parts[0].mesh->vertices[0];
            std::cout << "first uv: " << v.uv.v << ", " << v.uv.w << "\n";
        }
