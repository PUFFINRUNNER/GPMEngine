...
namespace P {
    ...
    public:
        ...
        Vector2D operator/(float x) const noexcept {  //divide magnitude by x
            if (x > -EPS && x < EPS) {Vector2D(std::numeric_limits<float>::quiet_NaN(),
        ...
        Vector2D normalized() const noexcept;//return a normalized vector with the same direction as this
        Vector2D &normalize() noexcept;   //normalize this vector
        float dot(const Vector2D& other) const noexcept; //dot product
        float cross(const Vector2D& other) const noexcept;   //cross product
        Vector2D rotated(float angle) const noexcept;    //return a rotated vector with the same magnitude
        Vector2D &rotate(float angle) noexcept;   //rotate this vector
        Vector2D perpendicular() const noexcept; //return a perpendicular vector
        Vector2D &makePerpendicular() noexcept;   //make this vector perpendicular to itself
        bool isParallel(const Vector2D &other) const noexcept; //check if this and other are parallel
        bool isPerpendicular(const Vector2D &other) const noexcept; //check if this and other are perpendicular
    };
