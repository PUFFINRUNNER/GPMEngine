...
namespace {
    [[nodiscard]] M::Point3D closestPointOnTriangle(const M::Point3D& p,
                                                    const M::Point3D& a,
                                                    const M::Point3D& b,
                                                    const M::Point3D& c) noexcept {
        const M::Vector3D ab = b - a;
        const M::Vector3D ac = c - a;
        const M::Vector3D ap = p - a;
    ...
    [[nodiscard]] bool pointInsideAABB(const M::Point3D& p, const M::AABB3D& box) noexcept {
        return p.x >= box.min.x && p.x <= box.max.x &&
            p.y >= box.min.y && p.y <= box.max.y &&
            p.z >= box.min.z && p.z <= box.max.z;
    }

    [[nodiscard]] float projectedRadiusAABB(const M::AABB3D& box, const M::Vector3D& n) noexcept {
        const M::Vector3D he = box.size() * 0.5f;
        return std::fabs(n.x) * he.x + std::fabs(n.y) * he.y + std::fabs(n.z) * he.z;
    }
...
namespace P {
    ...
    ContactManifold collide(Fixture& a, Fixture& b) noexcept {
        ...
        if (ta == ShapeType::Sphere && tb == ShapeType::TriangleMesh) {
            return collideSphereTriangleMesh(a, b);
        }

        if (ta == ShapeType::TriangleMesh && tb == ShapeType::Sphere) {
            ContactManifold m = collideSphereTriangleMesh(b, a);
            if (m.valid()) {
                std::swap(m.a, m.b);
                m.normal.negate();
                for (int i = 0; i < m.pointCount; ++i) {
        ...
        if (ta == ShapeType::Box && tb == ShapeType::TriangleMesh) {
            return collideBoxTriangleMesh(a, b);
        }

        if (ta == ShapeType::TriangleMesh && tb == ShapeType::Box) {
            ContactManifold m = collideBoxTriangleMesh(b, a);
            if (m.valid()) {
                std::swap(m.a, m.b);
                m.normal.negate();
                for (int i = 0; i < m.pointCount; ++i) {
    ...
    ContactManifold collideSphereTriangleMesh(Fixture& sphereFixture,
                                            Fixture& meshFixture) noexcept {
        ContactManifold manifold{};

        auto* sphere = dynamic_cast<SphereCollider*>(sphereFixture.collider.get());
        auto* mesh = dynamic_cast<TriangleMeshCollider*>(meshFixture.collider.get());
        if (!sphere || !mesh || mesh->empty()) {
            return manifold;
        }

        const M::Transform3D ts = sphereFixture.worldTransform();
        const M::Transform3D tm = meshFixture.worldTransform();

        const float radius = sphere->radius * maxAbsScaleComponent(ts.scale);
        const M::Point3D center = ts.position;

        float bestDist2 = std::numeric_limits<float>::max();
        M::Point3D bestPoint;
        M::Vector3D bestNormal(0.0f, 1.0f, 0.0f);
        bool found = false;
        ...
        for (const auto& face : mesh->faces) {
            ...
            const M::Point3D a = transformPoint(tm, mesh->vertices[static_cast<std::size_t>(face.a)]);
            const M::Point3D b = transformPoint(tm, mesh->vertices[static_cast<std::size_t>(face.b)]);
            const M::Point3D c = transformPoint(tm, mesh->vertices[static_cast<std::size_t>(face.c)]);

            M::Vector3D n = (b - a).cross(c - a);
            if (n.magnitudeSqr() <= M::EPS) {
                continue;
            }
            n.normalize();

            const M::Point3D q = closestPointOnTriangle(center, a, b, c);
            const float dist2 = center.distanceSqrTo(q);
        ...
        if (!found || bestDist2 > radius * radius) {
            return manifold;
        }

        const float dist = std::sqrt(std::max(0.0f, bestDist2));
        M::Vector3D normal = bestNormal;
        if (dist > M::EPS) {
            normal = (center - bestPoint).normalized();
        }

        manifold.a = &sphereFixture;
        manifold.b = &meshFixture;
        manifold.normal = normal;
        manifold.pointCount = 1;
        manifold.points[0].position = bestPoint;
        manifold.points[0].normal = normal;
        manifold.points[0].penetration = radius - dist;

        return manifold;
    }

    ContactManifold collideBoxTriangleMesh(Fixture& boxFixture,
                                        Fixture& meshFixture) noexcept {
        ContactManifold manifold{};

        auto* box = dynamic_cast<BoxCollider*>(boxFixture.collider.get());
        auto* mesh = dynamic_cast<TriangleMeshCollider*>(meshFixture.collider.get());
        if (!box || !mesh || mesh->empty()) {
            return manifold;
        }
        ...
        const M::AABB3D boxAabb = boxFixture.worldAABB();
        const M::Point3D boxCenter = boxAabb.center();
        ...
        for (const auto& face : mesh->faces) {
            ...
            // quick reject by triangle AABB
            M::AABB3D triBox(a, a);
            triBox.expandToInclude(b);
            triBox.expandToInclude(c);
            if (!M::intersects(triBox, boxAabb)) {
                continue;
            }

            const float signedDist = n.dot(boxCenter - a);
            const float radius = projectedRadiusAABB(boxAabb, n);
            const float penetration = radius - std::fabs(signedDist);

            if (penetration <= 0.0f) {
                continue;
            }

            const M::Point3D q = closestPointOnTriangle(boxCenter, a, b, c);

            // keep only contacts near or inside the box
            if (!pointInsideAABB(q, boxAabb)) {
                const M::Point3D clamped = boxAabb.clampedPoint(q);
                if (clamped.distanceSqrTo(q) > 0.05f * 0.05f) {
                    continue;
                }
            }
            ...
            if (penetration > bestPenetration) {
                ...
                bestPoint = q;
                bestNormal = (signedDist >= 0.0f) ? n : -n;
            }
        }
        ...
        manifold.pointCount = 1;
        manifold.points[0].position = bestPoint;
        manifold.points[0].normal = bestNormal;
        manifold.points[0].penetration = bestPenetration;
